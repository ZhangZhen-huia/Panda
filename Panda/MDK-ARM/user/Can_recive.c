...
static CAN_TxHeaderTypeDef Chassis_Header,Gimbal_Header,Lift_Header,Txmessage; 
...
void Can1_Receive_test(uint8_t data[]);
...
void CAN1_Send_Test(uint32_t msgid)
{
	uint32_t pT;
	...
	HAL_CAN_AddTxMessage(&hcan1,&Txmessage,&chassis_move.Paw_flag,&pT);
}

void Can1_Receive_test(uint8_t data[])
{
			chassis_move.uwb_data.x = data[1]*256+data[2];
			chassis_move.uwb_data.y	= data[3]*256+data[4];
			if(!first_flag)
			{
				first_C_yaw=(data[5]*256+data[6]);
				first_flag=1;
			}
			C_yaw = (data[5]*256+data[6]);//yaw_caculate(uwb_receive_data[4]*256+uwb_receive_data[5]);
			final_yaw = C_yaw_caculate();
